...
author: Fxhou
date: 2024-02-25
excerpt: MIT-Missing-Semester-第八节课-元编程，包含构建系统、依赖管理（版本号语义）、测试、持续集成
clipboard: true
key: 2024-02-25-metaprogramming
---
...
> Info "注意"
> 在构建大型系统时，可以将 `make` 视作 "胶水"，使用 `make` 调用其他工具。例如 makefile 先调用 cargo 来构建一个 rust 程序，然后调用 Cmake 来构建该 rust 程序所依赖的C语言程序库，在程序构建完运行 rust 代码中的单元测试，然后使用C语言代码进行绘图等。
...
> Warning "注意"
> "向后兼容"是指在引入变化或修改后，系统仍然能够与之前的版本或接口保持兼容性，而不需要对现有的客户端代码进行修改。换句话说，向后兼容性确保新版本的软件可以无缝地与旧版本的软件进行交互，而不会引发错误或破坏现有功能。
...
# git ls-files -o：列出Git仓库中未被跟踪（untracked）的文件。
# xargs：从标准输入中读取数据，并将其作为参数传递给后面的命令。
# rm -f：删除文件，-f参数表示强制删除，即使文件只读或不存在也执行删除操作。  
```

2. 基于 [GitHub Pages](https://pages.github.com/) 创建任意一个可以自动发布的页面。添加一个[GitHub Action](https://github.com/features/actions) 到该仓库，对仓库中的所有 shell 文件执行  `shellcheck`([方法之一](https://github.com/marketplace/actions/shellcheck))；
3. [构建属于您的](https://help.github.com/en/actions/automating-your-workflow-with-github-actions/building-actions) GitHub action，对仓库中所有的`.md`文件执行[`proselint`](http://proselint.com/) 或 [`write-good`](https://github.com/btford/write-good)，在您的仓库中开启这一功能，提交一个包含错误的文件看看该功能是否生效。