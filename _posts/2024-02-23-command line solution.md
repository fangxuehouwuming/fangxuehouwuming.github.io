...
我们可以**使用 [`fg`](https://www.man7.org/linux/man-pages/man1/fg.1p.html) 或 [`bg`](http://man7.org/linux/man-pages/man1/bg.1p.html) 命令恢复暂停的工作**。它们分别表示在前台继续或在后台继续。
...
还有一件事情需要掌握，那就是**命令中的 `&` 后缀可以让命令在直接在后台运行，这使得您可以直接在 shell 中继续做其他操作**，不过它此时还是会使用 shell 的标准输出，这一点有时会比较恼人（这种情况可以使用 shell 重定向处理）。
...
> info "注意"
> `kill -SIGHUP` 起到关闭终端的作用，因此，未使用nohup的进程1被终止，而进程2继续运行
...
不仅如此，**终端多路复用使我们可以分离当前终端会话并在将来重新连接（服务器）**。
...
- **会话** - 每个会话都是一个独立的工作区，其中包含一个或多个窗口
    ...
    + `tmux a` 重新连接最后一个会话。您也可以通过 `tmux attach -t NAME` 来指定具体的会话
