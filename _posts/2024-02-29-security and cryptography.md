...
[密钥生成函数](https://en.wikipedia.org/wiki/Key_derivation_function) (Key Derivation Functions, KDF) 作为密码散列函数的相关概念，被应用于包括生成固定长度，可以使用在其他密码算法中的密钥等方面。
...
encrypt(plaintext: array<byte>, key) -> array<byte>  (输出密文ciphertext)
decrypt(ciphertext: array<byte>, key) -> array<byte>  (输出明文plaintext)
```
...
- [PGP电子邮件加密](https://en.wikipedia.org/wiki/Pretty_Good_Privacy)：我可以将所使用的公钥在线发布，比如：PGP密钥服务器或
[Keybase](https://keybase.io/)。任何人都可以使用我公布的公钥向我发送加密的电子邮件，我使用私钥即可解密邮件。
...
# 对称加密和非对称加密的结合
![alt text](https://github.com/fangxuehouwuming/fangxuehouwuming.github.io/raw/master/blogImages/hybridEncrypt.png)  
...
[def]: /2020/command-line/#remote-machines